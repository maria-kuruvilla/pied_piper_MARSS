...
#add residuals of the linear model with temperature and photoperiod as a column

lm_temp_day <- lm(temp_day_inp ~ photoperiod_day, data = data)
lm_temp_night <- lm(temp_night_inp ~ photoperiod_night, data = data)
...
ggsave(here("puyallup", "output", "covariate_w_temp_correlations.png"), width = 10, height = 10, units = "in")
...
covariates_chinook0_w_temp <- arrange(data,doy) %>%
  filter(doy >130 & doy <= 218 & year > 2010) %>%
  dplyr::select(year,doy, flow_day_inp, flow_night_inp, 
                ...
                resid_day, resid_night, chinook0_hatchery_perhour_day_inp, chinook0_hatchery_perhour_night_inp) %>%
...
num_covariates = 10
total_covariates = dim(covariates_chinook0_w_temp)[1]


for(i in 1:(total_covariates/2)){ # everything except diffs and hatchery
  covariates_chinook0_w_temp[i,] = scale(covariates_chinook0_w_temp[i,])[,1]
}

#just scale

for(i in (total_covariates/2 + 1):(total_covariates)){
  covariates_chinook0_w_temp[i,] = scale(covariates_chinook0_w_temp[i,], center = FALSE, scale= TRUE)[,1]
}
...
for(kk in c(2,3,5,7,9)){
  c = covariates_chinook0_w_temp[((1+(kk-1)*num_rows):(kk*num_rows)),]
  name_long = rownames(covariates_chinook0_w_temp)[1+(kk-1)*num_rows]
...
write.csv(out.tab.all.years_ordered, file = here("puyallup","output",
                                                 "chinook_correlated_covariates_w_temp_selection.csv"))
...
for(i in 1:length(list_combinations)){
  ...
  for(j in covariates){
    ...
    c = rbind(c,covariates_chinook0_w_temp[((1+(k-1)*num_rows):(k*num_rows)),])
    name_long = rownames(covariates_chinook0_w_temp)[1+(k-1)*num_rows]
  ...
  out=data.frame(c=name, photoperiod_difference = photoperiod_difference,
                 photoperiod = photoperiod, temperature_difference = temperature_difference,
...
fits.hatchery.all.years[[97]]
ci_puyallup_chinook_new <- tidy(fits.hatchery.all.years[[97]])
ci_puyallup_chinook_new

ggplot(ci_puyallup_chinook_new[c(25:29),], 
...
ggsave(here("puyallup","output","chinook_best_model_w_temp.png"), width = 10, height = 6, units = "in")
...
relative_importance_temp_diff <- sum(out.tab.hatchery.all.years$weights[out.tab.hatchery.all.years$temperature_difference==1])
...
write.csv(riv_photo_diff_puyallup, file = here("puyallup",
                                               "output","riv_photo_diff_puyallup_w_temp.csv"))
...
write.csv(out.tab.hatchery.all.years.ordered, file = here("puyallup",
                                                          "output",
                                                          "model_selection_w_temp.csv"))

#calculate the cross correlation between chinook0_wild_perhour and chinook0_hatchery_perhour
...
ccf(ccf_data$chinook0_hatchery_perhour_inp,
    ccf_data$chinook0_wild_perhour_inp, lag.max = 10, plot = TRUE)
