#Goal - to re run marss analysis for chinook with both airtemp and atu
...
data %>%
  dplyr::select(flow_day,photoperiod_day, lunar_phase_day, secchi_depth_day, atu_day,
...
#save ggplot
ggsave(here("puyallup","output","puyallup_correlated_covariates_all_day.png"))

#night, rename all the columns remove night from it 
data %>%
  dplyr::select(flow_night,photoperiod_night, lunar_phase_night, secchi_depth_night, atu_night,
                photo_diff_night,flow_diff_night, temp_night, temp_diff_night, resid_night) %>%
  rename(flow = flow_night, photoperiod = photoperiod_night, lunar_phase = lunar_phase_night,
         secchi_depth = secchi_depth_night, atu = atu_night, photo_diff = photo_diff_night,
         flow_diff = flow_diff_night, temp = temp_night, temp_diff = temp_diff_night,
         resid = resid_night) %>%
  GGally::ggpairs(upper=list(continuous='points'),
                  lower=list(continuous='cor'))
ggsave(here("puyallup","output","puyallup_correlated_covariates_all_night.png"), width = 10, height = 10)
...
write.csv(out.tab.all.years_ordered, file = here("puyallup","output",
                                                 "chinook_correlated_covariates_selection_atu.csv"))
#photoperiod and temp - temp
#secchi_depth and photo diff - photo_diff
#secchi depth and temp - temp
#photo diff and resid - photo_diff
#temp and resid - resid
#photodiff and atu - photo_diff
###########

#the rest will be the same as the code in chinook0_w_airtemp
# 
